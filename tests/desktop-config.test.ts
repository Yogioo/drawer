...
test('exposes browser and desktop lifecycle commands', () => {
	...
	assert.equal(packageJson.scripts['test:security'], 'npm audit --audit-level=high')
	assert.equal(packageJson.scripts['test:installer'], 'node scripts/windows-installer-smoke.mjs')
	assert.match(packageJson.scripts['test:release'], /tsc -b/)
	assert.match(packageJson.scripts['test:release'], /test:security/)
	assert.match(packageJson.scripts['test:release'], /test:installer/)
	assert.match(smokeTest, /desktop:dev/)
	assert.match(smokeTest, /desktop:build/)
	assert.match(smokeTest, /unexpected sidecar/)
	assert.match(smokeTest, /bundle.*nsis/s)
})
